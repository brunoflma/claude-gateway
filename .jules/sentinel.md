...
## 2026-06-24 - Cross-Origin Leakage via CORS Allow-Credentials
**Vulnerability:** The proxy's CORS configuration unconditionally set `Access-Control-Allow-Credentials: true`. When combined with a `null` or `*` computed origin (e.g., for local desktop Office Add-in execution or untrusted fallbacks), this allows cross-origin requests via sandboxed iframes to access authenticated responses.
**Learning:** Returning `Access-Control-Allow-Credentials: true` is strictly forbidden by modern browsers when the origin is `*`. When the origin is `null`, it can still be exploited by attackers using sandboxed iframes (`<iframe sandbox="allow-scripts" src="...">`) to leak sensitive data because their requests carry a `null` origin.
**Prevention:** Conditionally omit the `Access-Control-Allow-Credentials: true` header when the returned origin is `null` or `*`. It should only be set for explicitly trusted and validated origins.
